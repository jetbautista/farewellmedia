$(document).ready(function() {
	...
	$('.input-check').on('click', function() {
		...
		$this.closest('.product2_item').find('.embed-radio').toggleClass('show');
		if ($this.is(':checked')) {
	...
	//Total Est. Price
	$('.input-check, .quantity-field, .radio-button').change(function() {
		var sum = 0;
		...
		$('.input-check').each(function() {
            ...
            windowOption = $(this).closest('.product2_item').find('.radio-button:checked').val() || '';
	...
	$('.input-check').on('click', function() {
		...
		var windowOption = $(this).closest('.product2_item').find('.radio-button:checked').val() || '';
	...
	$('input[required]').change(function() {
		if ($('input[type="text"]:required').val() &&
			...
			$('input[type="submit"]').on('click', function() {
				$('input[type="checkbox"], .button-radio, .quantity-field ').remove(); //Remove fields
			});
		} else {
			$('.form-submit').addClass('disabled');
