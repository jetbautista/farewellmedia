$(document).ready(function() {
	...
	$('.sliderwindow-option.w-condition-invisible').remove();

	//Total Est. Price
	$('.input-check, .quantity-field, .radio-field').change(function() {
		var sum = 0;
		...
		$('.input-check').each(function() {
			...
			originalPrice = parseFloat($(this).closest('.product2_item').find('.input-check').val());
            ...
            windowOption = $(this).closest('.product2_item').find('.radio-field:checked').val() || '';
	...
	$('.input-check').on('click', function() {
		...
		var productTotal = $(this).closest('.product2_item').find('.price').text();
		var windowOption = $(this).closest('.product2_item').find('.radio-field:checked').val() || '';
		var qtyProductPrice = quantity+'x '+productName + windowOption +': '+'$'+productTotal;
	...
	$('.quantity-field, .radio-field').on('change', function() {
		var productName = $(this).closest('.product2_item').find('.input-check').attr('name');
		var quantity = $(this).closest('.product2_item').find('.quantity-field').val();
		var productTotal = $(this).closest('.product2_item').find('.price').text();
		var windowOption = $(this).closest('.product2_item').find('.radio-field:checked').val() || '';
	...
	if (product) {
		...
		$('.specs-size').text(product.size);
		$('.specs-weight').text(product.weight);
	...
	$('input[required]').change(function() {
		if ($('input[type="text"]:required').val() &&
			...
			$('input[type="submit"]').on('click', function() {
				$('input[type="checkbox"], .radio-field, .quantity-field ').remove(); //Remove fields
			});
		} else {
			$('.form-submit').addClass('disabled');
